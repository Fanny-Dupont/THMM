...
# Distance above which we know narwhal can't be disturbed: 50km, 60km, 70km and 100km
indexQ = which(data$distance_km>=60)
...
fit_model <- function(i) {
  ...
  if(!skip_to_next) {
    ...
    # lbetaThr0= c(rnorm(1,-10,1),log(rnorm(1,2.5,3))) 
...
results = mclapply(1:Nrep, fit_model, mc.cores = 50)
...
saveRDS(m,"model_sensitivity_thr60.RData")
